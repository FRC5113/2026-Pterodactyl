...
class TrialErrorTuner:
    ...
    def start(
        self, module: SwerveWheel, initial_gains: Optional[AnalyticalGains] = None
    ) -> None:
        """
    ...
    def _apply_motor_config(self, **gains):
        ...
        status = self.current_module.direction_motor.configurator.apply(
            slot_config, 0.050
        )
    ...
    def _tune_kS(self, elapsed: float):
        ...
        if len(self.position_samples) > 10:
            ...
            if recent_movement > 0.01:  # Movement detected
                ...
                print(
                    f"Module {self.results.module_id} - Trial kS: {self.kS_trial:.4f}"
                )
                ...
                self._apply_motor_config(
                    kS=self.kS_trial, kV=self.kV_trial, kP=0.0, kI=0.0, kD=0.0
                )
                return
        ...
        if elapsed > 15.0 or (
            ...
            self.results.trial_kS = self.kS_trial
            print(
                f"Module {self.results.module_id} - Trial kS (timeout/sim): {self.kS_trial:.4f}"
            )
            self.current_test = "kV"
            self._reset_data()
            self._apply_motor_config(
    ...
    def _tune_kV(self, elapsed: float):
        ...
        if elapsed > 1.0 and len(self.velocity_samples) > 20:
            ...
            if (
                ...
                print(
                    f"Module {self.results.module_id} - Trial kV: {self.kV_trial:.4f} (error: {self.best_kV_error:.4f})"
                )

                self.current_test = "kP"
                self._reset_data()
                self._apply_motor_config(
                    kS=self.kS_trial, kV=self.kV_trial, kP=self.kP_trial, kI=0.0, kD=0.0
                )
                return
        ...
        if elapsed > 20.0 or (
            ...
            self.results.trial_kV = self.kV_trial
            print(
                f"Module {self.results.module_id} - Trial kV (timeout/sim): {self.kV_trial:.4f}"
            )
            self.current_test = "kP"
            self._reset_data()
            self._apply_motor_config(
    ...
    def _tune_kP(self, elapsed: float):
        ...
        if elapsed > 1.0 and len(self.position_samples) > 30:
            if self._detect_oscillation():
                ...
                print(
                    f"Module {self.results.module_id} - Trial kP: {self.kP_trial:.4f}"
                )
        ...
        if elapsed > 20.0 or (
            ...
            self.results.trial_kP = self.kP_trial
            print(
                f"Module {self.results.module_id} - Trial kP (timeout/sim): {self.kP_trial:.4f}"
            )
            self.current_test = "kD"
            self._reset_data()
            self._apply_motor_config(
    ...
    def _tune_kD(self, elapsed: float):
        ...
        if elapsed > 1.0 and len(self.velocity_samples) > 30:
            if self._detect_jitter():
                ...
                print(
                    f"Module {self.results.module_id} - Trial kD: {self.kD_trial:.4f}"
                )
        ...
        if elapsed > 20.0 or (
            ...
            print(
                f"Module {self.results.module_id} - Trial kD (timeout/sim): {self.kD_trial:.4f}"
            )
            self.is_done = True
            self.is_running = False
            self.current_test = "idle"
