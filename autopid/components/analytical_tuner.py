...
class AnalyticalTuner:
    ...
    def setup(self):
        ...
        # Test state
        self.current_test = (
            "idle"  # idle, static_friction, step_pos, step_neg, oscillation
        )
    ...
    def _test_static_friction(self, elapsed: float):
        ...
        self.current_module.direction_motor.set_control(controls.VoltageOut(voltage))
        ...
        if len(self.position_samples) > 10:
            ...
            if recent_movement > 0.01:  # ~0.57 degrees
                ...
                self.nt.put(
                    f"Module {self.results.module_id}/Static Friction V", voltage
                )
                print(
                    f"Module {self.results.module_id} - Static friction: {voltage:.3f}V"
                )
        ...
        if elapsed > 10.0 or (
            ...
            print(
                f"Module {self.results.module_id} - Static friction (timeout/sim): {voltage:.3f}V"
            )
            self.current_test = "step_pos"
            self.processing_positive_step = True
            self._reset_data()
    ...
    def _analyze_step_response(self, positive: bool):
        ...
        self.nt.put(
            f"Module {self.results.module_id}/Time Constant {suffix}",
            time_constant or 0.0,
        )
        self.nt.put(
            f"Module {self.results.module_id}/Max Velocity {suffix}", max_velocity
        )

        print(
            f"Module {self.results.module_id} - Step response ({suffix}): "
            f"tau={time_constant:.3f}s, max_vel={max_velocity:.3f}"
            if time_constant
            else f"max_vel={max_velocity:.3f}"
        )
    ...
    def _analyze_oscillations(self):
        ...
        if len(peaks) >= 2:
            ...
            self.nt.put(
                f"Module {self.results.module_id}/Oscillation Period", avg_period
            )
            self.nt.put(
                f"Module {self.results.module_id}/Oscillation Amplitude", avg_amplitude
            )

            print(
                f"Module {self.results.module_id} - Oscillations: "
    ...
    def _calculate_analytical_gains(self):
        ...
        print(
            f"Module {self.results.module_id} - Analytical gains: "
