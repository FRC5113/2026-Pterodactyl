...
class SwerveTuner(StateMachine):
    ...
    def run_analytical_tuning(self, initial_call):
        ...
        if initial_call:
            print(
                f"Starting analytical tuning for module {self.current_module.direction_motor.device_id}"
            )
        ...
        if self.analytical_tuner.is_complete():
            ...
            if module_id not in self.tuning_results:
                ...
                self.tuning_results[module_id].static_friction_voltage = (
                    results.static_friction_voltage
                )
                self.tuning_results[module_id].time_constant_pos = (
                    results.time_constant_pos
                )
                self.tuning_results[module_id].time_constant_neg = (
                    results.time_constant_neg
                )
                self.tuning_results[module_id].max_velocity_pos = (
                    results.max_velocity_pos
                )
                self.tuning_results[module_id].max_velocity_neg = (
                    results.max_velocity_neg
                )
                self.tuning_results[module_id].oscillation_period = (
                    results.oscillation_period
                )
                self.tuning_results[module_id].oscillation_amplitude = (
                    results.oscillation_amplitude
                )
    ...
    def run_trial_tuning(self, initial_call):
        ...
        if initial_call:
            print(
                f"Starting trial-and-error tuning for module {self.current_module.direction_motor.device_id}"
            )
